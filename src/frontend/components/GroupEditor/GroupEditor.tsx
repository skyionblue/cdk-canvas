import React, {useState} from 'react';
import {Node} from 'reactflow';
import './GroupEditor.css';

const GROUP_COLORS = [
  {name: 'Blue', value: '#dbeafe', border: '#3b82f6'},
  {name: 'Green', value: '#dcfce7', border: '#16a34a'},
  {name: 'Purple', value: '#f3e8ff', border: '#9333ea'},
  {name: 'Orange', value: '#ffedd5', border: '#ea580c'},
  {name: 'Pink', value: '#fce7f3', border: '#ec4899'},
  {name: 'Gray', value: '#f1f5f9', border: '#64748b'},
];

interface GroupEditorProps {
  group: Node;
  onUpdateGroup: (groupId: string, label: string, color: string) => void;
  onClose: () => void;
}

export function GroupEditor({group, onUpdateGroup, onClose}: GroupEditorProps) {
  const [label, setLabel] = useState(group.data.label || '');
  const [selectedColor, setSelectedColor] = useState(
    GROUP_COLORS.find((c) => c.border === group.data.color) || GROUP_COLORS[0],
  );

  const handleUpdate = () => {
    if (!label.trim()) {
      alert('Please enter a label for the group.');
      return;
    }
    onUpdateGroup(group.id, label.trim(), selectedColor.border);
    onClose();
  };

  return (
    <div className="group-editor-overlay" onClick={onClose}>
      <div className="group-editor" onClick={(e) => e.stopPropagation()}>
        <div className="group-editor-header">
          <h3>✏️ Edit Group</h3>
          <button className="close-button" onClick={onClose}>
            ✕
          </button>
        </div>

        <div className="group-editor-content">
          <div className="form-group">
            <label htmlFor="group-label">Group Label</label>
            <input
              id="group-label"
              type="text"
              value={label}
              onChange={(e) => setLabel(e.target.value)}
              placeholder="e.g., API Layer, Database Tier"
              maxLength={40}
              autoFocus
            />
          </div>

          <div className="form-group">
            <label>Group Color</label>
            <div className="color-grid">
              {GROUP_COLORS.map((color) => (
                <button
                  key={color.value}
                  className={`color-option ${selectedColor.value === color.value ? 'selected' : ''}`}
                  onClick={() => setSelectedColor(color)}
                  style={{
                    backgroundColor: color.value,
                    borderColor: color.border,
                  }}
                  title={color.name}
                >
                  <span className="color-name">{color.name}</span>
                </button>
              ))}
            </div>
          </div>
        </div>

        <div className="group-editor-actions">
          <button
            className="update-button"
            onClick={handleUpdate}
            disabled={!label.trim()}
          >
            Update Group
          </button>
          <button className="cancel-button" onClick={onClose}>
            Cancel
          </button>
        </div>
      </div>
    </div>
  );
}
